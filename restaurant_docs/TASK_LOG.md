...
任务目标：
- 将 `生成封面` 按钮放入中间列 `视频设置` 区域，位于 `当前本地图片数量：X 张` 下方。
- 复用视频设置中的本地上传图片，不再在封面验证区提供独立图片上传控件。
- 保留文案设置中的 `生成视频标题/视频文案` 和标题候选选择。

本轮调整：
- 移除左侧文案区域中的封面生成入口调用。
- 在视频设置的图片数量提示下方渲染 `生成封面` 按钮。
- 点击 `生成封面` 时：
  - 如果没有上传图片，显示 `请先上传图片` 类受控提示，不调用 pipeline。
  - 如果没有生成并选择标题，显示 `请先生成视频标题/视频文案并选择标题` 类受控提示，不调用 pipeline。
  - 如果图片和标题均满足，使用 `selected_cover_title_id`、`theme_text` 和视频设置上传图片写入仓库外临时 project，并调用本地 cover pipeline。
- 成功后继续展示 `cover_image.png`、封面标题、封面图片、渲染状态、输出尺寸、外部 API 调用状态和 warning 摘要。
