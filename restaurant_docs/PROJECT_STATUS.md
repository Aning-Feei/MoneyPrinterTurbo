...
- 当前下一步：人工播放 final-1.mp4，检查画面、字幕、语音、时长和餐厅图片顺序
...
人工播放 final-1.mp4，检查画面、字幕、语音、时长和餐厅图片顺序；决定是否将 storage/、生成视频、音频、字幕等加入 .gitignore。
...
已通过。本阶段已完成基础生成链路只读检查、WebUI 启动和浏览器冒烟测试，并定位到最终视频候选文件。
